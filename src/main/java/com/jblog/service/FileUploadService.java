...
public class FileUploadService {
	...
	public String store(MultipartFile multipartFile) {
		...
		try {
			...
			System.out.println("Multipart-원본파일명:" + originalFileName);
			System.out.println("Multipart-파일사이즈:" + size);
			//	확장자 분리
			String extName = originalFileName.substring(
					originalFileName.lastIndexOf("."));
			System.out.println("파일 확장자:" + extName) ;
			//	저장될 실제 파일명을 얻어오기
			saveFilename = getSaveFilename(extName);
			System.out.println("실제 저장될 파일명:" + saveFilename);
	...
	private void writeFile(MultipartFile mFile, 
		...
		System.out.println("fileData= getBytes"+ fileData);
		//	saveFilename에 저장
